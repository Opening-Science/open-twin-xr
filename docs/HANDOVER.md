...
>   not (Z-Anatomy). BodyParts3D carries no `_STRUCTURE` and so cannot be masked.
>
...
>
> **Known limitation worth picking up:** overlay superseding matches structures BY
> NAME, and Z-Anatomy names both ears' ossicles identically, so the one-sided ear
> overlay cannot hide the structures it stands in for without blanking the other
> ear too. Ontology terms are the fix, and `docs/ONTOLOGY_MAP.md` is the inventory
> for it — generated by `npm run gen:ontology`, so it cannot drift. Read its first
> two sections before touching overlays: the crosswalks cover 1,408 terms, but
> **Z-Anatomy and BodyParts3D carry none of them in the shipped asset**, which is
> what leaves overlays on name matching in the first place.
>
...
There is no linter. `npm run lint` was removed because eslint was never
installed and no config existed, so it only ever failed. The
`eslint-disable-next-line` comments in the source are inert. Adding eslint back
is a deliberate task — `npm install` re-resolves every caret range, which is how
this app has broken before.
...
|---|---|---|---|---|
| BodyParts3D | TARO, adult Japanese male, 2 mm MRI | **none** | FMA, 1,838 rows | The default. Merged to 11 draw calls, which is where the term was lost |
| Z-Anatomy | TARO again, retopologised | **none** | FMA, 676 rows | **3,614** structures across 7 system files in the shipped `--publishable` build; 3,617 in the full research build |
