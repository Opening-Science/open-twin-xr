...
>
> **One email to send** before anything derived from the beating heart is
> published — see the `biv-heart` entry in `licences.json`.
>
...
| ct-atlas-f — own geometry | unidentified — source scan not recorded | ⚠️ unresolved (MOOSE weights CC BY 4.0) | no table | **no — unresolved** |
| htb-ct-f — own geometry | TCIA Healthy-Total-Body-CTs collection authors | CC BY 4.0 | no table | yes |
| biv-heart — own geometry | University of Auckland Heart Mechanics Research Group | Apache-2.0 | 3 | **no — subject cohort unidentified — UK Biobank or CARDIOHANCE** |
